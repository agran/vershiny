...
_Тот же силуэт без камеры — вид с Приюта 11. У Восточной вершины кружок: она
...
- **Поворот по хвату** — интерфейс сам становится в ландшафт, когда телефон
  повёрнут набок (без системного поворота окна), и переворачивается при
  перехвате другим боком
...
| Рельеф, офлайн        | Copernicus GLO-90 → [agran/vershiny-dem](https://github.com/agran/vershiny-dem)           | 947 МБ, 217 м / 435 м / 1.7 км         |
| Рельеф детальный      | GLO-90 → [agran/vershiny-dem-hi](https://github.com/agran/vershiny-dem-hi) (~87 м)        | 957 МБ, приоритетные регионы целиком   |
| Вершины               | OpenStreetMap (`natural=peak`, `natural=volcano`)                                         | 692 355, из них 4 984 вулкана          |
| Регионы               | `tools/regions.json`                                                                      | 124 региона, двуязычные                |

175 ГБ исходных GeoTIFF сжимаются в две разреженные пирамиды: базовая на
947 МБ (детальный уровень достаётся горам приоритетных регионов, грубый
покрывает всю сушу) и детальный слой ~87 м на 957 МБ — отдельный репозиторий,
потому что лимит GitHub Pages на сайт — 1 ГБ. Подробности —
в [DATA-PIPELINE.md](docs/DATA-PIPELINE.md).
...
# GLO-90 → обе пирамиды тайлов (глобальная 217 м + детальная 87 м)
