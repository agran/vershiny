...
describe("снимок панорамы", () => {
  ...
  const captionLines = (): typeof draws =>
    draws.filter(
      (d) => !/^\S+ \d+°$/.test(d.text) && !d.text.includes("agran.github.io"),
    );
  ...
  it("подписи рисуются обводкой, без прямоугольной подложки", async () => {
    ...
    const plates = fillRects.filter(
      (r) =>
        !(r.x === 0 && r.y === 0 && r.w === shot.width && r.h === shot.height),
    );
  ...
  const captionWith = async (
    caption: Partial<PhotoCaption>,
  ): Promise<string[]> => {
