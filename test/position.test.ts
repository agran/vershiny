...
describe('положение наблюдателя', () => {
  ...
  it('без готового фикса старт идёт от точки прошлого запуска, не дожидаясь спутников', async () => {
    // Ровно случай «запустил в горах без сети»: A-GPS недоступен, холодный
    // фикс — это минуты, и всё это время висела заставка
    vi.useFakeTimers();
    ...
    // Ждём ровно быстрый запрос: до ожидания спутников дело не доходит
