  ...
  for (const c of caps) {
    ...
    if (c.side === "above" || c.side === "below") {
      ...
      const vertHits = (ry: number): boolean => {
        ...
        for (const o of caps) {
          ...
          if (horiz && self.top < b.top + b.height && self.bottom > b.top)
            return true;
        }
        return false;
      };
    ...
    const arrowHits = (rx: number, ry: number): boolean => {
      ...
      for (const o of caps) {
        ...
        if (
          Math.hypot(ocx - from.x, ocy - from.y) - orad < len &&
          segIntersectsRect(from, to, {
            left: ocx - orad,
            top: ocy - orad,
            right: ocx + orad,
      ...
      for (const [a2, b2] of arrows) if (segSeg(from, to, a2, b2)) return true;
      return false;
    };
      ...
      for (let radius = 0; radius <= 60 && !ok; radius++) {
        for (let dx = -radius; dx <= radius && !ok; dx++) {
          for (const dy of [-radius, radius]) {
            ...
            const tx = x + dx * 14;
            const ty = y + dy * 14;
            if (tx < 4 || tx > W - c.w - 4 || ty < 4 || ty > H - c.h - 4)
              continue;
...
describe("раскладка плашек подписей", () => {
  ...
  it("каждая плашка на своей стороне кнопки (стрелка короткая)", () => {
    for (const p of placed) {
      ...
      const b = cap.btn;
      if (cap.side === "right")
        expect(p.x, p.text).toBeGreaterThanOrEqual(b.left + b.width);
      if (cap.side === "left")
        expect(p.x + p.w, p.text).toBeLessThanOrEqual(b.left);
      if (cap.side === "above")
        expect(p.y + p.h, p.text).toBeLessThanOrEqual(b.top);
      if (cap.side === "below")
        expect(p.y, p.text).toBeGreaterThanOrEqual(b.top + b.height);
    }
  });
  ...
  it("стрелки не режут чужие кнопки", () => {
    ...
    placed.forEach((p, i) => {
      ...
      for (const btn of caps.map((c) => c.btn)) {
        if (btn === own) continue;
        const ocx = btn.left + btn.width / 2,
          ocy = btn.top + btn.height / 2,
          orad = btn.width / 2;
...
describe("segSeg: пересечение отрезков включая касание", () => {
  it("строгое пересечение", () => {
    ...
    expect(
      segSeg({ x: 0, y: 0 }, { x: 4, y: 4 }, { x: 6, y: 0 }, { x: 10, y: 4 }),
    ).toBe(false);
  });
  ...
  it("касание и общий конец — тоже пересечение", () => {
    ...
    expect(
      segSeg({ x: 0, y: 5 }, { x: 10, y: 5 }, { x: 5, y: 5 }, { x: 5, y: 10 }),
    ).toBe(true);
    // Общий конец
    expect(
      segSeg({ x: 0, y: 0 }, { x: 5, y: 5 }, { x: 5, y: 5 }, { x: 10, y: 0 }),
    ).toBe(true);
    // Коллинеарные с перекрытием
    expect(
      segSeg({ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 5, y: 0 }, { x: 15, y: 0 }),
    ).toBe(true);
    // Коллинеарные без перекрытия — мимо
    expect(
      segSeg({ x: 0, y: 0 }, { x: 4, y: 0 }, { x: 5, y: 0 }, { x: 9, y: 0 }),
    ).toBe(false);
...
function portraitCaps(): Cap[] {
  ...
  return [
    ...
    { btn: mk(330, 12), side: "left", ...text("Скачать регион для офлайна") },
    {
      btn: mk(330, 100),
      side: "left",
      ...text("Совместить вершины с камерой"),
    },
