...
describe('калибровка датчиков', () => {
  ...
  it('поправки переживают перезапуск', () => {
    ...
    const stored = normalizeCalibration(JSON.parse(store.get('vershiny-calibration')!));
  ...
  it('битое хранилище не ломает запуск', () => {
    expect(normalizeCalibration({ azimuthDeg: NaN, tiltDeg: undefined })).toEqual({
      azimuthDeg: 0,
      tiltDeg: 0,
      cameraFovDeg: null,
      autoCalibrate: true,
  ...
  it('автосовмещение включено по умолчанию и выключается', () => {
    ...
    expect(normalizeCalibration(JSON.parse(store.get('vershiny-calibration')!)).autoCalibrate).toBe(
      false,
    );
  });

  it('сброс возвращает расчётное поле зрения', () => {
    setCalibration({ cameraFovDeg: DEFAULT_CAMERA_FOV_DEG - 10, azimuthDeg: 5 });
