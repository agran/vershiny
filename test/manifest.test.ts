...
describe("манифест PWA", () => {
  ...
  it("по умолчанию — ландшафт: панорама смотрится в нём", () => {
    // Манифестный запрет — единственный способ получить ландшафт сразу при
    // запуске на Android без жеста. «any» отдал бы стартовую ориентацию
    // системе (портрет), и пришлось бы ждать клика по кнопке поворота
