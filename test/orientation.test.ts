...
describe("направление взгляда по датчику ориентации", () => {
  ...
  it("поворот в ландшафт не меняет азимут", () => {
    ...
    expect(deg(landscapeLeft.azimuthRad)).toBeCloseTo(
      deg(portrait.azimuthRad),
      1,
    );
    expect(deg(landscapeRight.azimuthRad)).toBeCloseTo(
      deg(portrait.azimuthRad),
      1,
    );
...
describe("выбор источника азимута", () => {
  it("при наличии абсолютного игнорирует относительное событие", () => {
    ...
    expect(
      isAbsoluteReading("deviceorientationabsolute", false, true, false),
    ).toBe(true);
    expect(isAbsoluteReading("deviceorientation", false, false, true)).toBe(
      false,
    );
  });

  it("пока абсолютного нет, берёт что есть", () => {
    expect(isAbsoluteReading("deviceorientation", false, false, false)).toBe(
      true,
    );
  });

  it("iOS: webkitCompassHeading абсолютен, хотя событие обычное", () => {
    expect(isAbsoluteReading("deviceorientation", true, false, true)).toBe(
      true,
    );
  });

  it("флаг absolute у обычного события тоже считается", () => {
    expect(isAbsoluteReading("deviceorientation", false, true, true)).toBe(
      true,
    );
  });
});
...
describe("комплементарный фильтр: коррекция дрейфа компасом", () => {
  ...
  it("большое расхождение принимается сразу: это смена системы отсчёта", () => {
    ...
    expect(deg(correctDrift(deg2rad(10), deg2rad(80), 0.016))).toBeCloseTo(
      80,
      6,
    );
  });
  ...
  it("без подтверждения (allowSnap=false) большое расхождение идёт экспонентой", () => {
    ...
    expect(degSigned(next)).toBeCloseTo(
      10 + 70 * (1 - Math.exp(-0.016 / 1.5)),
      3,
    );
  });
});
