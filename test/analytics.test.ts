...
describe("счётчик посещений", () => {
  ...
  it("карта кликов выключена: считаем посещения, а не движения пальцем", () => {
    ...
    const calls: unknown[][] = [];
    (window as unknown as { ym?: unknown }).ym = (...args: unknown[]) =>
      calls.push(args);
...
describe("адрес страницы для счётчика", () => {
  it("не выносит координаты из ссылки в статистику", () => {
    ...
    expect(
      pageUrlForCounter(
        "https://agran.github.io/vershiny/?lat=43.35&lon=42.44",
      ),
    ).toBe("https://agran.github.io/vershiny/");
    expect(
      pageUrlForCounter("https://agran.github.io/vershiny/install.html#top"),
    ).toBe("https://agran.github.io/vershiny/install.html");
  });

  it("различает страницы сайта — иначе считать было бы нечего", () => {
    const app = pageUrlForCounter(
      "https://agran.github.io/vershiny/?lat=1&lon=2",
    );
    const install = pageUrlForCounter(
