/**
 * Что подписывать на снимке (ROADMAP 4.4).
 *
 * По умолчанию — ничего: снимком делятся, а координаты с точностью до метра
 * и время съёмки — это данные о человеке, а не о горах. Снимок из дома,
 * отправленный в чат, не должен рассказывать, где этот дом. Кому подпись
 * нужна (маршрутный отчёт, привязка кадра к месту), включает её в настройках.
 *
 * Две независимые галочки, а не одна: место и время утекают по-разному.
 * В отчёте о восхождении дата уместна, а точка стоянки — не всегда.
 */
...
export const DEFAULT_PHOTO_CAPTION: PhotoCaption = { place: false, time: false };
...
export function normalizePhotoCaption(raw: Partial<PhotoCaption> | null): PhotoCaption {
  ...
  return { place: raw.place === true, time: raw.time === true };
}
