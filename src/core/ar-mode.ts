 ...
 *
 * Первый запуск на телефоне — включаем (и во вкладке, и в установленном
 * приложении): это главный режим, и разрешение всё равно придётся дать.
 * Отказ (или явный выход из AR) запоминается, и больше при каждой загрузке
 * камеру никто не просит: кнопка остаётся на месте.
 *
 * Исключения:
 *   - Mi Browser — камера в нём не работает вовсе (см. isMiBrowser);
 *   - прошивки, где автозапуск камеры убивает процесс (Xiaomi HyperOS),
 *     защищает сторож в main.ts: после одной погибшей сессии загрузка
 *     идёт без камеры, а успешное ручное включение возвращает автозапуск.
 */
