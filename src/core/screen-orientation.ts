 ...
 * не включается).
 *
 * Манифест ориентацию НЕ фиксирует: `"orientation": "landscape"`
 * убивал установленное приложение на Xiaomi HyperOS при запуске с
 * рабочего стола (из уведомления запускалось, но с жёстко ландшафтным
 * окном). Окно следует за системным автоповоротом, а ландшафт по хвату
 * даёт программный поворот — как во вкладке браузера.
 */
 ...
 *
 * Поворот включается ТОЛЬКО из портретного окна: этот механизм и
 * существует, чтобы дать ландшафт, которого система не дала. Если окно
 * уже ландшафтное (системный автоповорот вкладки или приложения),
 * доворачивать ничего не надо — и старый признак «форма окна ≠ форме
 * хвата» здесь НЕ работает: Android Chrome пересчитывает β/γ
 * deviceorientation в координаты ЭКРАНА, и в ландшафтном окне датчик
 * «выглядит портретно» при ландшафтном хвате. Поворот поверх системного
 * ландшафта давал двойное вращение: канвас становился портретным, и
 * контуры ложились на экран боком (видео и кнопки при этом выглядели
 * правильно — только они и были пересчитаны).
 */
export function syncOrientation(): boolean {
  if (!hasTilt) return false;
  applySoftRotation(
    effectiveOrientation() === "portrait" && tiltForm === "landscape",
  );
  return true;
}
 ...
 * поэтому в обычной вкладке этот вызов — честный no-op (там и так
 * работает системный автоповорот). В установленном приложении
 * (display: standalone) и в браузерах, где lock разрешён без fullscreen,
 * попытка срабатывает. Одна на сессию, отказ логируется, а не роняет
 * приложение.
 */
