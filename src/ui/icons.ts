...
/** Автоповорот: телефон, по бокам две дуги со стрелками в противоход */
export const ICON_ROTATE_AUTO = svg(
  // телефон по центру — достаточно широкий, чтобы читался между дугами
  '<rect x="9" y="6.5" width="6" height="11" rx="1.5"/>' +
    // верхняя дуга слева направо, стрелка вправо-вниз
    '<path d="M4 9a8 8 0 0 1 8-5"/>' +
    '<path d="M16 1.5 19 4l-4 1"/>' +
    // нижняя дуга справа налево, стрелка влево-вверх
    '<path d="M20 15a8 8 0 0 1-8 5"/>' +
    '<path d="M8 22.5 5 20l4-1"/>',
);

/**
 * Замочек для запертых режимов — контурный, отнесён от корпуса телефона:
 * вплотную к нему значок превращался в пятно
 */
const LOCK_BADGE =
  '<rect x="16" y="15.5" width="6" height="5" rx="1"/>' +
  '<path d="M17.6 15.5v-1.6a1.4 1.4 0 0 1 2.8 0v1.6"/>';

/** Заперто в ландшафте: горизонтальный телефон + замок сбоку */
export const ICON_ROTATE_LANDSCAPE = svg(
  '<rect x="2.5" y="8" width="12" height="9" rx="1.5"/>' + LOCK_BADGE,
);

/** Заперто в портрете: вертикальный телефон + замок сбоку */
export const ICON_ROTATE_PORTRAIT = svg(
  '<rect x="5.5" y="4" width="8" height="11" rx="1.5"/>' + LOCK_BADGE,
);
...
/**
 * Глобус — метка выбора языка.
 *
 * Подпись у этой строки бесполезна ровно тому, кому она нужнее всех:
 * человек, попавший в чужой язык интерфейса, не прочитает ни «Язык», ни
 * «Language». Значок опознаётся без чтения.
 */
...
/**
 * Ручка направления взгляда на карте: стрелка «от себя».
 *
 * Размер меньше остальных: кружок ручки 32 px, и полноразмерная иконка
 * упёрлась бы в его края.
 */
export const ICON_HEADING = svg(
  '<path d="M12 4.5v15"/><path d="m7.5 9 4.5-4.5L16.5 9"/>',
  18,
);
