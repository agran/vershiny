...
async function compute(
  ...
  const layered = computeLayeredHorizon(origin, observerH, sampleFn, {
    marchDeps,
    // Офлайн: обрезаем хвост лучей за пределами скачанных тайлов — дальняя
    // зона (30–200 км) там отвечает NaN, и до 2/3 шагов марша тратились
    // впустую. Зонд смотрит уже загруженные тайлы, поэтому обрезка точна
    coverageProbe: source.isOffline
      ? (pos: LatLon) => source.mayHaveOfflineData(pos)
      : undefined,
