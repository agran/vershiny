...
| Файл                 | Содержание                                                                                                                                                                           |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **STATUS.md**        | **Актуальное состояние**: что работает, известные и решённые проблемы, архитектура данных, размеры, производительность, следующие шаги. **Обновлять при каждом значимом изменении!** |
| **ROADMAP.md**       | План работ: этапы 0–3 (MVP → v1.1 → v2.0) с отметками о выполнении, критерии приёмки MVP, риски                                                                                      |
| **ARCHITECTURE.md**  | Архитектура: Canvas 2D (без WebGL), Web Worker, DemSource (патч → Terrarium → пирамида → кеш), система координат проекции                                                            |
| **ALGORITHMS.md**    | Алгоритмы: ray-marching (3600 лучей, кривизна Земли k=0.13), видимость пиков, калибровка компаса, изоляция вершин, поиск, перелёт                                                    |
| **DATA-PIPELINE.md** | Данные: трёхслойная схема геоданных, форматы тайлов, глобальная пирамида GLO-90 (скачивание и сборка), вершины из OSM, реестр регионов, хранение на устройстве                       |
| **CONTRIBUTING.md**  | Как контрибьютить: принципы, код-стайл, стек                                                                                                                                         |
